...
规律：x是一个5维向量，向量中哪一维的标量最小就返回哪一维的向量下标
...
def evaluate(model):
    ...
    y1=y2=y3=y4=y5=0
    for y_value in y:
        if y_value==torch.tensor([4]):
            y5+=1
        elif y_value==torch.tensor([3]):
            y4+=1
        elif y_value==torch.tensor([2]):
            y3+=1
        elif y_value==torch.tensor([1]):
            y2+=1
        else:
            y1+=1

    print("本次预测集中共有%d个一类样本，%d个二类样本，%d个三类样本，%d个四类样本，%d个五类样本" % (y1,y2,y3,y4,y5))
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测
